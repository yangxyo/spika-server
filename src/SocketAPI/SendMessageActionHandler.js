  ...
  socket.on("sendMessage", function (param) {
    ...
    if (Utils.isEmpty(param.roomID)) {
      socket.emit("socketerror", {
        code: Const.resCodeSocketSendMessageNoRoomID,
      });
      return;
    }
    ...
    if (Utils.isEmpty(param.userID)) {
      socket.emit("socketerror", {
        code: Const.resCodeSocketSendMessageNoUserId,
      });
      return;
    }
    ...
    if (Utils.isEmpty(param.type)) {
      socket.emit("socketerror", {
        code: Const.resCodeSocketSendMessageNoType,
      });
      return;
    }
    ...
    if (param.type == Const.messageTypeText && Utils.isEmpty(param.message)) {
      socket.emit("socketerror", {
        code: Const.resCodeSocketSendMessageNoMessage,
      });
      return;
    }
    ...
    if (
      ...
      socket.emit("socketerror", {
        code: Const.resCodeSocketSendMessageNoLocation,
      });
