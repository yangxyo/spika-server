...
MessageModel.prototype.init = function () {
  ...
  this.model = mongoose.model(
    Settings.options.dbCollectionPrefix + "messages",
    messageSchema,
  )
  return this.model
}
...
MessageModel.prototype.findAllMessages = function (
  roomID,
  lastMessageID,
  callBack,
) {
...
MessageModel.prototype.findMessages = function (
  roomID,
  lastMessageID,
  limit,
  callBack,
...
MessageModel.prototype.populateMessages = function (messages, callBack) {
  ...
  if (ids.length > 0) {
    UserModel.findUsersbyInternalId(ids, function (err, userResult) {
      ...
      _.forEach(
        messages,
        function (messageElement, messageIndex, messagesEntity) {
